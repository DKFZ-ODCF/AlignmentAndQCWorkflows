...
def mod_bconv_samcomment(sysargv):
	import sys
	import argparse
	import pysam
	import re
	import datetime
	def nicetime(): return datetime.datetime.now().strftime("[bconv_samcomment %Y-%m-%d %H:%M:%S]")
	...
	parser.add_argument("-t", "--tag", dest="tag", action="store", default="ZC", help="SAM tag to read the conversion information from")
	...
	try:
		...
		for tid in range(len(h["SQ"])):										# header should be in the order of reference file generated by faconv, otherwise exit
			if tid%2==0:
				if h["SQ"][tid]["SN"][-2:] != "_W": sys.exit("methylCtools bconv_samcomment: error: invalid BAM header")
				hseqnew.append(h["SQ"][tid])
				hseqnew[-1]["SN"] = h["SQ"][tid]["SN"][:-2]
			else:
				if h["SQ"][tid]["SN"][-2:] != "_C": sys.exit("methylCtools bconv_samcomment: error: invalid BAM header")
		h["SQ"] = hseqnew
		...
		sys.exit("methylCtools bconv_samcomment: error: %s" % strerror)
	...
	def convert(p, s, r):													# p: conversion positions, s: sequence, r: is_reverse
		...
		for i in pp[:-1]:
			...
			else: sys.exit("methylCtools bconv_samcomment: error: unexpected conversion")
 		return s
	...
	while 1:
		...
		if read1.is_paired:
			...
			if read1.qname != read2.qname: sys.exit("methylCtools bconv_samcomment: error: input read order: %s, %s" % (read1.qname, read2.qname))
