...
set -xv
set +u
# Don't set -u: The following code won't work with existing methylation files,
# because then too many arrays are empty and thus unbound with buggy Bash versions.
...
# WARNING: May result in unbound variable error with `set -u` and older Bash versions < 4.4.
...
for n in $(seq 0 "$MAX"); do
	...
	if [[ ! "$f" -gt "$r" ]]; then
		...
		if [[ ! -f "$FILE_FRONT_CP" ]] || [[ ! -f "$FILE_BACK_CP" ]]; then
			if [[ "$forward_index" -ne "$reverse_index" ]]; then
...
# Wait for the methylation calling processes to be done and check whether all checkpoint files were created.
