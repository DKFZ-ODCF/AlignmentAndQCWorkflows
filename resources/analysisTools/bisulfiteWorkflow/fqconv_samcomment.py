...
def mod_fqconv_samcomment(sysargv):
	import sys
	import argparse
	import gzip
	import datetime
	def nicetime(): return datetime.datetime.now().strftime("[fqconv_samcomment %Y-%m-%d %H:%M:%S]")
	
	
	#######################################
	# arguments, filehandles

	parser = argparse.ArgumentParser(prog="methylCtools fqconv_samcomment", version="0.9.2", description="converts bisulfite sequencing reads to fully converted state")
	parser.add_argument("-s", "--silent", dest="qf", action="store_false", help="do not show status messages")
	parser.add_argument("-t", "--tag", dest="tag", action="store", default="ZC", help="SAM tag to store the conversion information in")
	...
	try:
		...
		sys.exit("methylCtools fqconv_samcomment: error: %s" % strerror)
	...
	for l1 in in1:
		...
		if n == 1:															# line 1: name
			...
			# Process the read entry after parsing the fourth line and write to output.
			# Methylation sites are encoded as positions followed by a sequence of continously occurring conversions.
			# E.g. 6CC means at 0-based position 6 to subsequent Cytosines were converted.
			comment1 = ""
			scount = 0             # zero-based indexing
			for s in seq1:
				if s == cfrom:
					if scount > 0: comment1 += "%i" % scount
					scount = 0
					comment1 += "%s" % cfrom
					c[1] += 1
				else: scount += 1
			...
			if len(comment1) >= 250: # if comment too long (this might actually not be necessary and is legacy from coding in identifiers!)
				comment1 = ""
				comment1 += "%i" % len(seq1)
			...
			if len(comment1) > 0:
				comment1 = args.tag + ":A:" + comment1
				o1 += ("%s %s\n%s\n+\n%s\n" % (id1, comment1, seq1, qual1))
			else:
				o1 += ("%s\n%s\n+\n%s\n" % (id1, seq1, qual1))
			n = 0															# reset counter
