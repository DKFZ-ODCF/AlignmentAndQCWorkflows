...
# Nov 25 update: skip the start and end positions for metrics
					...
					if p in dp:
						...
						if dp[p][0] == "+":												# position strand
							...
							for pr in pileupcolumn.pileups:
								...
								SKIP=False	# whether to skip the position when counting methylated residue
								if args.se:												# skip bases close to end of read
									...
									# Tagmentation-based bisulfite-seq specific
									if pr.alignment.is_read2:
										if pr.alignment.is_reverse: # reverse
											if pr.qpos >= pr.alignment.rlen-TAGMEN_SKIP_POS:
												SKIP=True
										else: # foward read2
											if pr.qpos < TAGMEN_SKIP_POS: # pro.qpos is 0-based;TAGMEN_SKIP_POS is 1-based
											      SKIP=True
									else: # 1st read
										if pr.alignment.is_reverse: # read1 reverse
											if abs(pr.alignment.isize)-(pr.alignment.qend-pr.qpos)<TAGMEN_SKIP_POS:
												SKIP=True
										else: # read1 forward
											if abs(pr.alignment.isize)-pr.qpos<=TAGMEN_SKIP_POS:
												SKIP=True
									...
									if pr.alignment.seq[pr.qpos+q] in ill[strand][q]:	# check for illegal bases
										...
										if q == 0 and not strand:
											if pr.alignment.seq[pr.qpos] == "C":
												...
												if args.me and not SKIP : dometrics(0,qf)
											else:			
												# already checked for illegal bases, only C/T is possible
												# if qf: ctcount[1] += 1
												if qf and not SKIP : ctcount[1] += 1
												# if args.me: dometrics(1)
												if args.me and not SKIP : dometrics(1,qf)
