...
if [[ "$bamFileExists" == "true" ]]; then
	...
	if [[ "$useBioBamBamSort" == "true" ]]; then # Biobambam makes the index
		cat "$NP_BAMSORT" \
		    | tee "$NP_COVERAGEQC_IN" "$NP_READBINS_IN" "$NP_FLAGSTATS" \
		    > "$tempSortedBamFile" \
		    & registerPid
		...
		($BWA_BINARY mem -t "$BWA_MEM_THREADS" \
	...
	else  	# We use samtools for making the index
...
groupLongAndShortChromosomeNames "$FILENAME_GENOME_COVERAGE.tmp" \
