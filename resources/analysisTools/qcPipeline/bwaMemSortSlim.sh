...
FILENAME_DIFFCHROM_PLOT_TMP=$(getFilePathOnRoddyScratch "${FILENAME_DIFFCHROM_PLOT}")
${RSCRIPT_BINARY} ${TOOL_PLOT_DIFFCHROM} -i "$FILENAME_DIFFCHROM_MATRIX" -s "$FILENAME_DIFFCHROM_STATISTICS" -o "${FILENAME_DIFFCHROM_PLOT_TMP}" && mv ${FILENAME_DIFFCHROM_PLOT_TMP} ${FILENAME_DIFFCHROM_PLOT} || ( echo "Error from chrom_diff.r" && exit 23)
