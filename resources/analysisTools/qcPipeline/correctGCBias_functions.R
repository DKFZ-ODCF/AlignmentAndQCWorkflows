...
checkControl <- function(coverage, covIndex){
  ...
  for (chr in sort( unique( as.numeric(coverage$chromosome) ) ) ){
    ...
    maxPeak <- zeroCrossings[which(dens$y[zeroCrossings]==max(dens$y[zeroCrossings])) ]
...
plotCoverage <- function(coverageTab, chromosomeBorders=NULL, chr=NULL, ylims=c(-4,4) ){
    ...
    plot(coverageTab$start/1e6, log2(coverageTab$covNnorm), xlab='genomic coordinate (MB)', ylab=' log2 corrected normalized coverage control', cex=0.01, main=paste0( "chr", chr, "\n", "Control coverage"), ylim=ylims )
    abline(h=0, col='red')
    if ( ! is.null(chromosomeBorders) ){
      abline( v=chromosomeBorders/1e6, lty='dotted' )
      text(x= labelPosition, y= 3.5, labels=as.character(c(1:22, 'X', 'Y')), cex=1.5 )
    }
    
    plot(coverageTab$start/1e6, log2(coverageTab$covTnorm), xlab='genomic coordinate (MB)', ylab='log2 corrected normalized coverage tumor', cex=0.01, main='Tumor coverage',  ylim=ylims )
    abline(h=0, col='red')
    if ( ! is.null(chromosomeBorders) ){
      abline( v=chromosomeBorders/1e6, lty='dotted' )
      text(x= labelPosition, y= 3.5, labels=as.character(c(1:22, 'X', 'Y')), cex=1.5 )
    ...
    plot(coverageTab$start/1e6, log2(coverageTab$covR), xlab='genomic coordinate (MB)', ylab='log2 corrected normalized coverage ratio', cex=0.01, main='Tumor/Control coverage ratio',  ylim=ylims)
    abline(h=0, col='red')
    if ( ! is.null(chromosomeBorders) ){
      abline( v=chromosomeBorders/1e6, lty='dotted' )
      text(x= labelPosition, y= 3.5, labels=as.character(c(1:22, 'X', 'Y')), cex=1.5 )
...
plotCoverageSingle <- function(coverageTab, chromosomeBorders=NULL, chr=NULL, ylims=c(-4,4) ){
  ...
  plot(coverageTab$start/1e6, log2(coverageTab$covnorm), xlab='genomic coordinate (MB)', ylab=' log2 corrected normalized coverage control', cex=0.01, main=paste0( "chr", chr, "\n", "Control coverage"), ylim=ylims )
  abline(h=0, col='red')
  if ( ! is.null(chromosomeBorders) ){
    abline( v=chromosomeBorders/1e6, lty='dotted' )
    text(x= labelPosition, y= 3.5, labels=as.character(c(1:22, 'X', 'Y')), cex=1.5 )
