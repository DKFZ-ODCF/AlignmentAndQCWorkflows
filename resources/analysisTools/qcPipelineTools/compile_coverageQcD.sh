...
​# ldc2-0.12.1-linux-x86_64/bin/rdmd --compiler=ldc2-0.12.1-linux-x86_64/bin/ldmd2 --build-only -IBioD-master -O -release -version=1 -inline coverageQcD/coverageQc.d
​
...
​fi
​
​$dcomp --compiler=$ldmd --build-only -I${bioD} -O -release -inline coverageQcD/coverageQc.d
​
​if [[ "$?" == 0 ]]
​then
​  mv coverageQcD/coverageQc $outfile && chmod 755 $outfile && echo "CoverageQc successfully compiled and moved to $outfile" && exit 0
​else
​  rm $outfile
