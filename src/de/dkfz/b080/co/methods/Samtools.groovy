...
class Samtools {
    ...
    @ScriptCallingMethod
    static OnTargetCoveragePlotFile createOnTargetCoveragePlot(ExecutionContext run,
                                                               OnTargetCoverageTextFile textFile) {
        ...
        parameters.put("FILENAME_PREFIX", (textFile.fileStage as COFileStageSettings).IDString)
        parameters.put(ONTARGETCOVERAGEPLOTTER_BINARY,
                run.configuration.getProcessingToolPath(run,
                        ONTARGETCOVERAGEPLOTTER_BINARY
                ).absolutePath)

        BEJobResult jobResult =
                new Job(run,
                        run.createJobName(textFile, ONTARGETCOVERAGEPLOT),
                        ONTARGETCOVERAGEPLOT,
                        parameters,
                        [(BaseFile) textFile]).
                        run()
